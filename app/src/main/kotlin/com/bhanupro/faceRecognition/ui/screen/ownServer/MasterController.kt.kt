...
class MasterController(
    ...
    fun startSendingAudio(ctx: Context) {
        ...
        // 🔹 Try common sample rates (16kHz first, then 44.1kHz if not working well)
        ...
        for (rate in sampleRates) {
            try {
                ...
                if (bufSize != AudioRecord.ERROR_BAD_VALUE) {
                    bufferSize = (bufSize * 2).coerceAtLeast(2048) // safer buffer
        ...
        if (audioRecord == null) {
            uiToast("Failed to init AudioRecord")
            return
        }
        ...
        // ✅ Apply audio effects
        val sessionId = audioRecorder!!.audioSessionId
        if (AcousticEchoCanceler.isAvailable()) {
            AcousticEchoCanceler.create(sessionId)?.enabled = true
            logI(TAG_AUDIO, "AEC enabled")
        }
        if (NoiseSuppressor.isAvailable()) {
            NoiseSuppressor.create(sessionId)?.enabled = true
            logI(TAG_AUDIO, "NoiseSuppressor enabled")
        }
        if (AutomaticGainControl.isAvailable()) {
            AutomaticGainControl.create(sessionId)?.enabled = true
            logI(TAG_AUDIO, "AGC enabled")
        }

        try {
            audioRecorder?.startRecording()
            isRecording = true
            val usedBuffer = bufferSize
            val usedRate = chosenRate

            audioThread = thread(start = true, isDaemon = true) {
                val buf = ByteArray(usedBuffer)
                while (isRecording &&
                    audioRecorder != null &&
                    audioRecorder!!.recordingState == AudioRecord.RECORDSTATE_RECORDING
                ) {
                    val read = try {
                        audioRecorder!!.read(buf, 0, buf.size)
                    } catch (e: Exception) {
                        -1
                    }
                    if (read > 0) {
            ...
            logI(TAG_AUDIO, "Audio recording started (rate=$usedRate, buffer=$usedBuffer)")
        } catch (e: Exception) {
