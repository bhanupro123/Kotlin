package com.bhanupro.faceRecognition.ui.screen.addFace
...
import com.bhanupro.faceRecognition.lib.AiModel.mobileNet
...
class AddFaceViewModel @Inject constructor(private val repo: Repository) : ViewModel() {
    ...
    fun onRoleChange(value: String) = runCatching { image.value = image.value.copy(role = value) }.onFailure { LOG.e(it, it.message) }

    fun saveFace() = viewModelScope.launch {
        runCatching {
            hideSaveDialog()
            val error = withContext(Dispatchers.IO) { repo.saveFace(image.value).exceptionOrNull() }
            LOG.e("QWERTYinside", error?.message)
            snackbarHost.showSnackbar(error?.message ?: "Face Saved Successfully")
        }.onFailure {
            it.message?.let { it1 -> SnackbarManager.showMessage(it1) }
            LOG.e("QWERTY", it.message) }
    }
